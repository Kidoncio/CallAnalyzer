...
<!-- INÍCIO OBTENÇÃO DAS CHAMADAS ATENDIDAS -->
<?php
if($dePrint != $atePrint){
  $resultado = mysqli_query($conexao, "SELECT * FROM queue_log WHERE DATE(time) BETWEEN '$de' AND '$ate' AND event = 'CONNECT' ORDER BY time DESC");
  $rowResultado = mysqli_query($conexao, "SELECT COUNT(*) AS contagem FROM queue_log WHERE DATE(time) BETWEEN '$de' AND '$ate' AND event = 'CONNECT' ORDER BY time DESC");
  $rowResultado = mysqli_fetch_assoc($rowResultado);
  $rowResultado = $rowResultado['contagem'];
}
else{
  $resultado = mysqli_query($conexao, "SELECT * FROM queue_log WHERE DATE(time) = '$de' AND event = 'CONNECT' ORDER BY time DESC");
  $rowResultado = mysqli_query($conexao, "SELECT COUNT(*) AS contagem FROM queue_log WHERE DATE(time) = '$de' AND event = 'CONNECT' ORDER BY time DESC");
  $rowResultado = mysqli_fetch_assoc($rowResultado);
  $rowResultado = $rowResultado['contagem'];
}
?>
<!-- FIM OBTENÇÃO DAS CHAMADAS ATENDIDAS -->
  ...
  <div class="x_content">
    ...
    <div class="cold-md-10 col-sm-10 col-xs-10">
      ...
      if($de > $ate){ ?>
        ...
        <form action="formulario-atendidas.php" data-parsley-validate class="form-horizontal form-label-left">
        ...
        <h3 class="text-success">Ligações Atendidas</h3>
                  ...
                  <tr>
                    ...
                    <td>
                      <form action="resultado-agent.php" method="post">
                        <input type="hidden" name="agentid" value="<?=$chamadas['agent']?>" />
                        <button class="btn btn-info"><?=$chamadas['agent']?></button>
                      </form>
                    </td>
                    <!-- FIM AGENT -->
