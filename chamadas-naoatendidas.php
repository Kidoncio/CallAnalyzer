...
<!-- INÍCIO OBTENÇÃO DAS CHAMADAS NÃO ATENDIDAS -->
<?php
if($dePrint != $atePrint){
  $resultado = mysqli_query($conexao, "SELECT * FROM queue_log WHERE (DATE(time) BETWEEN '$de' AND '$ate') AND (event = 'ABANDON' OR event = 'EXITWITHTIMEOUT') ORDER BY time DESC");
  $rowResultado = mysqli_query($conexao, "SELECT COUNT(*) AS contagem FROM queue_log WHERE (DATE(time) BETWEEN '$de' AND '$ate') AND (event = 'ABANDON' OR event = 'EXITWITHTIMEOUT') ORDER BY time DESC");
  $rowResultado = mysqli_fetch_assoc($rowResultado);
  $rowResultado = $rowResultado['contagem'];
}
else{
  $resultado = mysqli_query($conexao, "SELECT * FROM queue_log WHERE DATE(time) = '$de' AND (event = 'ABANDON' OR event = 'EXITWITHTIMEOUT') ORDER BY time DESC");
  $rowResultado = mysqli_query($conexao, "SELECT COUNT(*) AS contagem FROM queue_log WHERE DATE(time) = '$de' AND (event = 'ABANDON' OR event = 'EXITWITHTIMEOUT') ORDER BY time DESC");
  $rowResultado = mysqli_fetch_assoc($rowResultado);
  $rowResultado = $rowResultado['contagem'];
}
?>
<!-- FIM OBTENÇÃO DAS CHAMADAS NÃO ATENDIDAS -->
  ...
  <div class="x_content">
    ...
    <div class="cold-md-10 col-sm-10 col-xs-10">
      ...
      if($de > $ate){ ?>
        ...
        <form action="formulario-naoatendidas.php" data-parsley-validate class="form-horizontal form-label-left">
        ...
        <h3 class="text-danger">Ligações Não Atendidas</h3>
